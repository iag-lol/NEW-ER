...
import { getSinTarjeta, updateSinTarjetaStatus, type SinTarjeta } from "@/lib/api/asistencia"
import { useToast } from "@/components/ui/use-toast"
import { exportToExcel } from "@/lib/export"

export function SinTarjetaTable({ refreshTrigger }: { refreshTrigger: number }) {
    const [data, setData] = useState<SinTarjeta[]>([])
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const records = await getSinTarjeta()
            setData(records)
        } catch (error) {
            console.error(error)
        } finally {
    ...
    const handleStatusChange = async (id: string, status: "Aprobado" | "Rechazado" | "Observado") => {
        try {
            await updateSinTarjetaStatus(id, status)
