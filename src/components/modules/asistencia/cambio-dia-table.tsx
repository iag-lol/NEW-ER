...
import { Check, X, Eye, MessageSquare, FileText, Download } from "lucide-react"
...
import { getCambioDia, updateCambioDiaStatus, type CambioDia } from "@/lib/api/asistencia"
import { useToast } from "@/components/ui/use-toast"
import { exportToExcel } from "@/lib/export"

export function CambioDiaTable({ refreshTrigger }: { refreshTrigger: number }) {
    const [data, setData] = useState<CambioDia[]>([])
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const records = await getCambioDia()
            setData(records)
        } catch (error) {
            console.error(error)
        } finally {
    ...
    const handleStatusChange = async (id: string, status: "Aprobado" | "Rechazado" | "Observado") => {
        try {
            await updateCambioDiaStatus(id, status)
